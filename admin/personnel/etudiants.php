...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    $groupe_sanguin = trim($_POST['groupe_sanguin']) ?? null;
    ...
    try {
        ...
        if (isset($_FILES['photo_profil']) && $_FILES['photo_profil']['error'] == UPLOAD_ERR_OK) {
            ...
            $file_name = uniqid('student_') . '_' . basename($_FILES['photo_profil']['name']);
        ...
        if ($_POST['action'] === 'add') {
            ...
            $sql = "INSERT INTO utilisateurs (nom, prenom, email, password, role, telephone, adresse, sexe, date_naissance, groupe_sanguin, photo_profil, statut) 
                    VALUES (?, ?, ?, ?, 'etudiant', ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$nom, $prenom, $email, $password, $telephone, $adresse, $sexe, empty($date_naissance) ? null : $date_naissance, $groupe_sanguin, $photo_path, $statut]);
            $message = "L'étudiant a été ajouté avec succès !";
            $message_type = 'success';

        } elseif ($_POST['action'] === 'edit') {
            $sql = "UPDATE utilisateurs SET nom = ?, prenom = ?, email = ?, telephone = ?, adresse = ?, sexe = ?, date_naissance = ?, groupe_sanguin = ?, photo_profil = ?, statut = ? WHERE id = ? AND role = 'etudiant'";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$nom, $prenom, $email, $telephone, $adresse, $sexe, empty($date_naissance) ? null : $date_naissance, $groupe_sanguin, $photo_path, $statut, $id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_delete'])) {
    try {
        ...
        $sql = "DELETE FROM utilisateurs WHERE id = ? AND role = 'etudiant'";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$id]);
        $message = "L'étudiant a été supprimé avec succès.";
        $message_type = 'success';
    } catch (PDOException $e) {
        $message = "Erreur : Impossible de supprimer cet étudiant, il est probablement lié à des inscriptions ou des notes.";
        $message_type = 'danger';
    }
}
...
try {
    $etudiants = $pdo->query("SELECT * FROM utilisateurs WHERE role = 'etudiant' ORDER BY nom, prenom ASC")->fetchAll();
} catch (PDOException $e) {
    $error_db = "Erreur lors de la récupération des étudiants : " . $e->getMessage();
    $etudiants = [];
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Gestion des Étudiants - GestiSchool Galaxy</title>
...
<div class="page-wrapper">
            ...
            <ul class="nav flex-column">
                ...
                <li class="nav-item">
                    ...
                    <div class="collapse show" id="personnelCollapse">
                        <ul class="nav flex-column ps-4">
                            <li><a class="nav-link" href="enseignants.php">Enseignants</a></li>
                            <li><a class="nav-link active" href="etudiants.php">Étudiants</a></li>
                        </ul>
                    </div>
                </li>
                <!-- ... autres menus ... -->
    ...
    <main id="main-content">
        <header class="main-header">
            ...
            <button class="btn btn-glow" data-bs-toggle="modal" data-bs-target="#etudiantModal" id="add-etudiant-btn">
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <?php foreach ($etudiants as $etu): ?>
                                <tr>
                                    <td>
                                        <?php if (!empty($etu['photo_profil'])): ?>
                                            <img src="../../<?= htmlspecialchars($etu['photo_profil']) ?>" alt="Photo de profil" class="profile-pic">
                                        <?php else: ?>
                                            <div class="profile-pic-default"><i class="fas fa-user-graduate"></i></div>
                                        <?php endif; ?>
                                    </td>
                                    <td><?= htmlspecialchars($etu['nom'] . ' ' . $etu['prenom']) ?></td>
                                    <td><?= htmlspecialchars($etu['email']) ?></td>
                                    <td><?= htmlspecialchars($etu['telephone'] ?: 'N/A') ?></td>
                                    <td class="text-center">
                                        <span class="status-badge status-<?= $etu['statut'] ?>">
                                            <?= $etu['statut'] ? 'Actif' : 'Inactif' ?>
                                        </span>
                                    </td>
                                    <td class="text-end">
    ...
    data-id="<?= $etu['id'] ?>"
    data-nom="<?= htmlspecialchars($etu['nom'] ?? '') ?>"
    data-prenom="<?= htmlspecialchars($etu['prenom'] ?? '') ?>"
    data-email="<?= htmlspecialchars($etu['email'] ?? '') ?>"
    data-telephone="<?= htmlspecialchars($etu['telephone'] ?? '') ?>"
    data-adresse="<?= htmlspecialchars($etu['adresse'] ?? '') ?>"
    data-sexe="<?= htmlspecialchars($etu['sexe'] ?? '') ?>"
    data-date_naissance="<?= htmlspecialchars($etu['date_naissance'] ?? '') ?>"
    data-groupe_sanguin="<?= htmlspecialchars($etu['groupe_sanguin'] ?? '') ?>"
    data-statut="<?= $etu['statut'] ?? '' ?>"
    data-photo_profil="<?= htmlspecialchars($etu['photo_profil'] ?? '') ?>">
    <i class="fas fa-pencil-alt"></i>
</button>

                                        <button class="btn btn-action btn-action-delete delete-btn" data-id="<?= $etu['id'] ?>" data-nom="<?= htmlspecialchars($etu['prenom'] . ' ' . $etu['nom']) ?>">
...
<!-- Modale pour Ajouter/Modifier un étudiant -->
        ...
        <div class="modal-content">
            <form method="POST" action="etudiants.php" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3"><label class="form-label">Date de Naissance</label><input type="date" class="form-control" id="form-date_naissance" name="date_naissance"></div>
                        <div class="col-md-4 mb-3"><label class="form-label">Groupe Sanguin</label><input type="text" class="form-control" id="form-groupe_sanguin" name="groupe_sanguin"></div>
                        <div class="col-md-4 mb-3"><label class="form-label">Statut</label><select class="form-select" id="form-statut" name="statut" required><option value="1">Actif</option><option value="0">Inactif</option></select></div>
                    </div>
...
<form method="POST" action="etudiants.php" id="delete-form" class="d-none">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const modalTitle = document.getElementById('etudiantModalLabel');
    ...
    document.getElementById('add-etudiant-btn').addEventListener('click', function () {
    ...
    document.querySelectorAll('.edit-btn').forEach(button => {
        button.addEventListener('click', function () {
            ...
            document.getElementById('form-date_naissance').value = data.date_naissance;
            document.getElementById('form-groupe_sanguin').value = data.groupe_sanguin;
    ...
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function () {
            const etudiantName = this.dataset.nom;
            const etudiantId = this.dataset.id;
            if (confirm(`Êtes-vous sûr de vouloir supprimer l'étudiant "${etudiantName}" ? Cette action est irréversible.`)) {
                document.getElementById('id-to-delete').value = etudiantId;
