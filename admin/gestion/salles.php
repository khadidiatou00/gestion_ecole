...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    $type = $_POST['type']; // 'classe', 'labo', 'sport', 'autre'

    try {
        if ($_POST['action'] === 'add') {
            $sql = "INSERT INTO salles (nom, batiment, capacite, type) VALUES (?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$nom, $batiment, $capacite, $type]);
            $message = "La salle a été ajoutée avec succès !";
            $message_type = 'success';
        } elseif ($_POST['action'] === 'edit' && isset($_POST['id'])) {
            $id = filter_input(INPUT_POST, 'id', FILTER_VALIDATE_INT);
            $sql = "UPDATE salles SET nom = ?, batiment = ?, capacite = ?, type = ? WHERE id = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$nom, $batiment, $capacite, $type, $id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_delete'])) {
    try {
        ...
        if ($e->getCode() == '23000') {
             $message = "Erreur : Impossible de supprimer cette salle car elle est utilisée dans un emploi du temps.";
        } else {
...
try {
    ...
    $error_db = "Erreur lors de la récupération des salles : " . $e->getMessage();
    $salles = [];
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Gestion des Salles - GestiSchool Galaxy</title>
...
<div class="page-wrapper">
                ...
                <li class="nav-item">
                    ...
                    <div class="collapse show" id="gestionCollapse">
                        <ul class="nav flex-column ps-4">
                            ...
                            <li><a class="nav-link active" href="salles.php">Salles</a></li>
    ...
    <main id="main-content">
        <header class="main-header">
            ...
            <button class="btn btn-glow" data-bs-toggle="modal" data-bs-target="#salleModal" id="add-salle-btn">
                        ...
                        <?php else: ?>
                            <?php foreach ($salles as $salle): ?>
                                <tr>
                                    ...
                                    <td><?= ucfirst(htmlspecialchars($salle['type'])) ?></td>
                                    <td class="text-end">
                                        <button class="btn btn-action btn-action-edit edit-btn"
                                            data-bs-toggle="modal" data-bs-target="#salleModal"
                                            data-id="<?= $salle['id'] ?>"
                                            data-nom="<?= htmlspecialchars($salle['nom']) ?>"
                                            data-batiment="<?= htmlspecialchars($salle['batiment']) ?>"
                                            data-capacite="<?= $salle['capacite'] ?>"
                                            data-type="<?= $salle['type'] ?>">
                                            <i class="fas fa-pencil-alt"></i>
                                        </button>
                                        <button class="btn btn-action btn-action-delete delete-btn" data-id="<?= $salle['id'] ?>" data-nom="<?= htmlspecialchars($salle['nom']) ?>">
...
<div class="modal fade" id="salleModal" tabindex="-1" aria-labelledby="salleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="salles.php">
                <div class="modal-header">
                    <h5 class="modal-title" id="salleModalLabel">Ajouter une salle</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="nom" class="form-label">Nom / Numéro de la salle</label>
                        <input type="text" class="form-control" id="form-nom" name="nom" required>
                    </div>
                    <div class="mb-3">
                        <label for="batiment" class="form-label">Bâtiment</label>
                        <input type="text" class="form-control" id="form-batiment" name="batiment">
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="capacite" class="form-label">Capacité</label>
                            <input type="number" class="form-control" id="form-capacite" name="capacite" required>
                        </div>
                        <div class="col-md-6 mb-3">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-glow" id="form-submit-btn">Ajouter la salle</button>
                </div>
            </form>
        </div>
    </div>
...
<form method="POST" action="salles.php" id="delete-form" class="d-none">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const modalTitle = document.getElementById('salleModalLabel');
    const formAction = document.getElementById('form-action');
    const formId = document.getElementById('form-id');
    const formSubmitBtn = document.getElementById('form-submit-btn');
    
    document.getElementById('add-salle-btn').addEventListener('click', function () {
        modalTitle.textContent = 'Ajouter une salle';
        formAction.value = 'add';
        formId.value = '';
        document.getElementById('salleModal').querySelector('form').reset();
        formSubmitBtn.textContent = 'Ajouter la salle';
    });
    ...
    document.querySelectorAll('.edit-btn').forEach(button => {
        button.addEventListener('click', function () {
            ...
            document.getElementById('form-batiment').value = data.batiment;
            document.getElementById('form-capacite').value = data.capacite;
    ...
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function () {
            const salleName = this.dataset.nom;
            const salleId = this.dataset.id;
            if (confirm(`Êtes-vous sûr de vouloir supprimer la salle "${salleName}" ? Cette action est irréversible.`)) {
                document.getElementById('id-to-delete').value = salleId;
