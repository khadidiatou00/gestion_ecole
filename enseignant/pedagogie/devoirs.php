...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        ...
        if (isset($_FILES['fichier']) && $_FILES['fichier']['error'] == UPLOAD_ERR_OK) {
            $upload_dir = '../../uploads/devoirs/'; // Dossier spécifique pour les devoirs
            ...
            $file_name = uniqid('devoir_') . '_' . basename($_FILES['fichier']['name']);
            $target_file = $upload_dir . $file_name;
            move_uploaded_file($_FILES['fichier']['tmp_name'], $target_file);
            $fichier_path = 'uploads/devoirs/' . $file_name;
        }
        ...
        if ($_POST['action'] === 'add') {
            ...
            $stmt->execute([$titre, $type, $date_limite, $matiere_id, $classe_id, $enseignant_id, $fichier_path, $annee_en_cours_id]);
            $message = "Le devoir a été ajouté avec succès !";
            $message_type = 'success';
        } elseif ($_POST['action'] === 'edit') {
            $sql = "UPDATE devoirs SET titre = ?, type = ?, date_limite = ?, matiere_id = ?, classe_id = ?, fichier_path = ? WHERE id = ? AND enseignant_id = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$titre, $type, $date_limite, $matiere_id, $classe_id, $fichier_path, $id, $enseignant_id]);
            $message = "Le devoir a été modifié avec succès !";
            $message_type = 'success';
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_delete'])) {
    try {
        ...
        $stmt_file = $pdo->prepare("SELECT fichier_path FROM devoirs WHERE id = ? AND enseignant_id = ?");
        ...
        $stmt = $pdo->prepare("DELETE FROM devoirs WHERE id = ? AND enseignant_id = ?");
        $stmt->execute([$id, $enseignant_id]);
        $message = "Le devoir a été supprimé avec succès.";
        $message_type = 'success';
    } catch (PDOException $e) {
...
try {
    ...
    $sql_devoirs = "SELECT d.*, cl.nom as classe_nom, m.nom as matiere_nom FROM devoirs d JOIN classes cl ON d.classe_id = cl.id JOIN matieres m ON d.matiere_id = m.id WHERE d.enseignant_id = ? AND d.annee_id = ? ";
    $params = [$enseignant_id, $annee_en_cours_id];
    if (!empty($_GET['classe_id'])) { $sql_devoirs .= " AND d.classe_id = ?"; $params[] = $_GET['classe_id']; }
    if (!empty($_GET['matiere_id'])) { $sql_devoirs .= " AND d.matiere_id = ?"; $params[] = $_GET['matiere_id']; }
    $sql_devoirs .= " ORDER BY d.date_limite ASC";
    
    $stmt_devoirs = $pdo->prepare($sql_devoirs);
    $stmt_devoirs->execute($params);
    $devoirs = $stmt_devoirs->fetchAll();

} catch (PDOException $e) {
    $error_db = "Erreur : " . $e->getMessage();
    $devoirs = $affectations = $classes_enseignant = $matieres_enseignant = [];
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Gestion des Devoirs - GestiSchool Vibrant</title>
    ...
    <style>
        ...
        .devoir-card { background: var(--card-bg); border-left: 5px solid var(--primary); border-radius: 12px; padding: 1.5rem; transition: all 0.3s ease; box-shadow: 0 4px 6px rgba(0,0,0,0.05); }
        .devoir-card.expired { border-left-color: var(--secondary); }
        .devoir-card:hover { transform: translateY(-5px); box-shadow: 0 10px 20px rgba(0,0,0,0.08); }
        .devoir-card h5 { font-family: var(--font-title); font-weight: 600; }
        .devoir-card .badge { font-weight: 500; }
...
<div class="page-wrapper">
    ...
    <aside id="sidebar">
        ...
        <nav class="sidebar-nav">
            <ul class="nav flex-column">
                ...
                <li class="nav-item"><a class="nav-link active" href="devoirs.php"><i class="fas fa-file-signature"></i> Devoirs</a></li>
    ...
    <main id="main-content">
        <header class="main-header">
            <h1 class="d-flex align-items-center"><i class="fas fa-file-signature me-3 text-primary"></i> Gestion des Devoirs</h1>
            <button class="btn btn-primary-glow" data-bs-toggle="modal" data-bs-target="#devoirModal" id="add-devoir-btn"><i class="fas fa-plus me-2"></i> Nouveau Devoir</button>
        </header>
        ...
        <div class="content-card mb-4">
            <form method="GET" action="devoirs.php" class="row g-3">
                ...
                <div class="col-md-2 d-flex align-items-end"><a href="devoirs.php" class="btn btn-secondary w-100">Réinitialiser</a></div>
            </form>
        </div>

        <div class="row g-4">
            <?php if (empty($devoirs)): ?>
                <div class="col-12"><p class="text-center text-muted mt-5">Aucun devoir trouvé pour cette sélection.</p></div>
            <?php else: ?>
                <?php foreach ($devoirs as $d): 
                    $date_limite = new DateTime($d['date_limite']);
                    $is_expired = $date_limite < new DateTime();
                ?>
                <div class="col-md-6 col-lg-4">
                    <div class="devoir-card <?= $is_expired ? 'expired' : '' ?>">
                        <h5><?= htmlspecialchars($d['titre']) ?></h5>
                        <div class="mb-3">
                            <span class="badge bg-primary bg-opacity-10 text-primary me-2"><?= htmlspecialchars($d['classe_nom']) ?></span>
                            <span class="badge bg-info bg-opacity-10 text-info"><?= htmlspecialchars($d['matiere_nom']) ?></span>
                        </div>
                        <p class="mb-2"><strong>Type :</strong> <?= ucfirst($d['type']) ?></p>
                        <p class="date-limite <?= $is_expired ? 'expired' : '' ?>"><i class="fas fa-calendar-alt me-2"></i>Date limite : <?= $date_limite->format('d/m/Y H:i') ?></p>
                        <div class="d-flex justify-content-end gap-2 mt-3">
                             <?php if ($d['fichier_path']): ?><a href="../../<?= htmlspecialchars($d['fichier_path']) ?>" class="btn btn-sm btn-outline-success" target="_blank" title="Télécharger l'énoncé"><i class="fas fa-download"></i></a><?php endif; ?>
                            <button class="btn btn-sm btn-outline-primary edit-btn" data-bs-toggle="modal" data-bs-target="#devoirModal" data-devoir='<?= json_encode($d) ?>'><i class="fas fa-pencil-alt"></i></button>
                            <button class="btn btn-sm btn-outline-danger delete-btn" data-id="<?= $d['id'] ?>"><i class="fas fa-trash-alt"></i></button>
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
...
<div class="modal fade" id="devoirModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="POST" action="devoirs.php" enctype="multipart/form-data">
                <div class="modal-header"><h5 class="modal-title" id="devoirModalLabel">Nouveau Devoir</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body">
                    ...
                    <div class="mb-3"><label class="form-label">Titre du devoir</label><input type="text" class="form-control" id="form-titre" name="titre" required></div>
                    <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3"><label class="form-label">Type de devoir</label><select class="form-select" name="type" id="form-type" required><option value="ecrit">Écrit</option><option value="oral">Oral</option><option value="projet">Projet</option></select></div>
                        <div class="col-md-6 mb-3"><label class="form-label">Date limite</label><input type="datetime-local" class="form-control" name="date_limite" id="form-date_limite" required></div>
                    </div>
                    <div class="mb-3"><label class="form-label">Énoncé (Fichier PDF, DOCX...)</label><input type="file" class="form-control" name="fichier" id="form-fichier"></div>
                </div>
...
<form method="POST" action="devoirs.php" id="delete-form" class="d-none">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const modal = document.getElementById('devoirModal');
    const modalTitle = document.getElementById('devoirModalLabel');
    const form = modal.querySelector('form');
    const formAction = document.getElementById('form-action');
    const formId = document.getElementById('form-id');
    const formSubmitBtn = document.getElementById('form-submit-btn');

    document.getElementById('add-devoir-btn').addEventListener('click', function () {
        modalTitle.textContent = 'Nouveau Devoir';
        form.reset();
        formAction.value = 'add';
        formId.value = '';
        document.getElementById('form-existing_file').value = '';
        formSubmitBtn.textContent = 'Ajouter le devoir';
    });
    
    document.querySelectorAll('.edit-btn').forEach(button => {
        button.addEventListener('click', function () {
            const data = JSON.parse(this.dataset.devoir);
